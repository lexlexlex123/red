...
const LAYOUTS=[
  ...
  // ── 6. HALO ── концентрические кольца, расходящиеся из центра
  {
    ...
    _build(w,h,a1,a2,isTitle,doAnimate){
      ...
      const swTable=isTitle?[28,3.5,18,8,30,5,14]:[22,3,16,7,26];
...
function _slideTplPreviewBg(){
  try{
    ...
    if(s){
      ...
      if(typeof _resolveSlideColorBg === 'function'){
...
/** Обновить только фон карточек шаблонов (без пересборки SVG). */
function refreshSlideTplPreviewBgs(){
  const themeBg = _slideTplPreviewBg();
  document.querySelectorAll('#slide-tpl-grid .slide-tpl-item').forEach(btn => {
