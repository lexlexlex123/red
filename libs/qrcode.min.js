// QR Code Library for browser — based on qrcode npm package
// Supports UTF-8, versions 1-10, EC level M
...
(function(){
  window.qrGenerate = function(text){
    const m = window.QRCodeLib.create(text, {errorCorrectionLevel:'M'});
    const size = m.modules.size;
    const rows = [];
    for(let r=0;r<size;r++){
      const row=[];
      for(let c=0;c<size;c++) row.push(m.modules.data[r*size+c]?1:0);
      rows.push(row);
    }
    return rows;
  };
  ...
  window.QRCode.prototype._generate=function(){
    const m=window.qrGenerate(this.text);
    let canvas=this.el.querySelector('canvas');
    if(!canvas){canvas=document.createElement('canvas');this.el.appendChild(canvas);}
    canvas.width=this.width; canvas.height=this.height;
    const ctx=canvas.getContext('2d');
    const size=m.length, cell=this.width/size;
    ctx.fillStyle='#fff'; ctx.fillRect(0,0,this.width,this.height);
    ctx.fillStyle='#000';
    for(let r=0;r<size;r++) for(let c=0;c<size;c++)
      if(m[r][c]) ctx.fillRect(c*cell,r*cell,cell,cell);
  };
})();
