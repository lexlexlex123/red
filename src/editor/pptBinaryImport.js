/**
 * Binary PPT (PowerPoint 97–2003) importer — React ESM port of js/26b-ppt-binary.js.
 * PersistDirectory → slide containers → ClientAnchor + text + pib images + hyperlinks.
 * No autoplace/autofit. Uses real slide size (often 4:3), scaled to React canvas.
 */
import { DEFAULT_CANVAS_W, DEFAULT_CANVAS_H } from './canvasDims.js';

export function isOleCompound(buf) {
  try {
  ...
  kids(u, a, b).forEach(function(r){
    if(r.type === 0xF004){ // SpContainer
      ...
      kids(u, r.dataOff, r.dataEnd).forEach(function(c){
        ...
        } else if(c.type === 0xF000 && c.len >= 8){
          // ClientAnchor - position data
          const dv = new DataView(u.buffer, u.byteOffset, u.byteLength);
          if(c.len >= 16) {
            // Full grid format: flag(2) + colL(2) + dxl(2) + rowL(2) + dyt(2) + colR(2) + dxr(2) + rowR(2) + dyb(2)
            const flag = dv.getInt16(c.dataOff, true);
            const colL = dv.getInt16(c.dataOff + 2, true);
            const dxl = dv.getInt16(c.dataOff + 4, true);
            const rowL = dv.getInt16(c.dataOff + 6, true);
            const dyt = dv.getInt16(c.dataOff + 8, true);
            const colR = dv.getInt16(c.dataOff + 10, true);
            const dxr = dv.getInt16(c.dataOff + 12, true);
            const rowR = dv.getInt16(c.dataOff + 14, true);
            const dyb = c.len >= 18 ? dv.getInt16(c.dataOff + 16, true) : 0;
            // Store grid coords for later conversion
            shape.anchor = {
              grid: true,
              colL: colL, dxl: dxl,
              rowL: rowL, dyt: dyt,
            ...
            // Short format: l(2) + t(2) + w(2) + h(2) - treat as master units
            shape.anchor = {
        ...
        } else if(c.type === 0xF010 && c.len >= 8 && !shape.anchor){
          // ClientData fallback - legacy format
          const dv = new DataView(u.buffer, u.byteOffset, u.byteLength);
          shape.anchor = {
            l: dv.getInt16(c.dataOff, true),
            t: dv.getInt16(c.dataOff + 2, true),
        ...
        } else if(c.type === 0xF00F && c.len >= 16 && !shape.anchor){
          // ChildAnchor fallback (for grouped shapes)
          const dv = new DataView(u.buffer, u.byteOffset, u.byteLength);
          shape.anchor = {
            l: dv.getInt32(c.dataOff, true),
            t: dv.getInt32(c.dataOff + 4, true),
            w: dv.getInt32(c.dataOff + 8, true) - dv.getInt32(c.dataOff, true),
            h: dv.getInt32(c.dataOff + 12, true) - dv.getInt32(c.dataOff + 4, true)
          };
        } else if(c.type === 0xF010 && c.len >= 8 && !shape.anchor){
          // ClientData fallback - 16-bit values
          const dv = new DataView(u.buffer, u.byteOffset, u.byteLength);
          shape.anchor = {
            l: dv.getInt16(c.dataOff, true),
            t: dv.getInt16(c.dataOff + 2, true),
...
function parsePptBinary(buf){
  ...
  const slideW = ar === '16:9' ? DEFAULT_CANVAS_W : 1920;
  const slideH = ar === '16:9' ? DEFAULT_CANVAS_H : 1440;
  const scX = slideW / sx;
  const scY = slideH / sy;
  ...
  function addShapeSlide(shapes){
    ...
    shapes.forEach(function(sh){
      if(!sh.anchor) return;
      // Convert grid coordinates to master units if needed
      let anchorL, anchorT, anchorW, anchorH;
      if(sh.anchor.grid) {
        // Grid-based format: convert to master units
        // Standard PPT grid uses 80 columns x 60 rows for a slide
        // Each cell is divided into 1024 sub-units
        const gridCols = 80;
        const gridRows = 60;
        const cellW = sx / gridCols;
        const cellH = sy / gridRows;
        anchorL = Math.round(sh.anchor.colL * cellW + (sh.anchor.dxl / 1024) * cellW);
        anchorT = Math.round(sh.anchor.rowL * cellH + (sh.anchor.dyt / 1024) * cellH);
        const r = Math.round(sh.anchor.colR * cellW + (sh.anchor.dxr / 1024) * cellW);
        const b = Math.round(sh.anchor.rowR * cellH + (sh.anchor.dyb / 1024) * cellH);
        anchorW = r - anchorL;
        anchorH = b - anchorT;
      } else {
        // Already in master units
        anchorL = sh.anchor.l;
        anchorT = sh.anchor.t;
        anchorW = sh.anchor.w;
        anchorH = sh.anchor.h;
      }
      let x = Math.round(anchorL * scX);
      let y = Math.round(anchorT * scY);
      let w = Math.round(Math.abs(anchorW) * scX);
      let h = Math.round(Math.abs(anchorH) * scY);
      // clamp into slide
      if(w < 8 || h < 8) return;
      ...
      if(!texts.length){
        // No text — check if it's a decorative shape
        const shapeType = sh.shapeType;
        // Map PPT shape types to React shape types
        // msosptRectangle=1, msosptRoundRectangle=2, msosptEllipse=9, etc.
        let shapeKind = null;
        if(shapeType != null) {
          if(shapeType === 1 || shapeType === 0) shapeKind = 'rect';
          else if(shapeType === 2) shapeKind = 'rect'; // roundRect
          else if(shapeType === 9) shapeKind = 'ellipse';
          else if(shapeType === 5 || shapeType === 6) shapeKind = 'triangle';
          else if(shapeType >= 12 && shapeType <= 19) shapeKind = 'arrow';
          else if(shapeType === 4 || shapeType === 10) shapeKind = 'rect'; // diamond, etc.
        }
        if(shapeKind) {
          els.push({ kind:'shape', shape: shapeKind, x:x, y:y, w:w, h:h, link: link });
          return;
        }
      ...
      const fsScale = slideW / 1200;
      els.push({
        kind:'text', x:x, y:y, w:w, h:h,
        text: text,
        fs: Math.round((isTitle ? 34 : (h < 40 * fsScale ? 16 : 20)) * fsScale),
        fw: isTitle ? '700' : '400',
  ...
  return parsed.slides.map(function(raw, si){
    ...
    (raw.els || []).forEach(function(e){
      if(e.kind === 'text'){
        ...
        const fw = e.fw || '400';
        // Wrap in <b> if bold so preserveInlineWeight can detect it
        const html = lines.map(function(ln){
          const escaped = escHtml(ln);
          return '<div>' + (fw === '700' ? '<b>' + escaped + '</b>' : escaped) + '</div>';
        }).join('');
        const fs = e.fs || 20;
        ...
        // Shape element — colors will be reset by resetImportedTextFormatting
        const d = {
          id:'e'+(ec++), type:'shape',
          shape: e.shape || 'rect',
          x:e.x, y:e.y, w:e.w, h:e.h,
          fill: '#3b82f6', stroke: '#1d4ed8', sw: 2,
          fillOp: 1, rot: 0, anims: [], rx: 0
        };
        if(e.link){ d.link = e.link; d.linkt = '_blank'; }
        els.push(d);
      }
...
<<<<<<<< HEAD:src/editor/pptBinaryImport.js
/**
 * @param {ArrayBuffer|Uint8Array} buf
 * @param {{filename?:string}} [opts]
 * @returns {Promise<{slides:object[], title:string, canvasW:number, canvasH:number, ar:string}>}
 */
export async function importPptBinaryFile(buf, opts = {}) {
  const parsed = parsePptBinary(buf);
  const slides = pptBinaryToSlides(parsed);
  const title = String(opts.filename || '')
    .replace(/\.ppt$/i, '')
    .split(/[/\\]/)
    .pop() || slides[0]?.title || 'Import';
  return {
    slides,
    title,
    canvasW: parsed.slideW,
    canvasH: parsed.slideH,
    ar: parsed.ar,
  };
========
...
>>>>>>>> f8bd18bab168c67e6e63057c1e751b81f629b97f:js/26b-ppt-binary.js
}

export { parsePptBinary, pptBinaryToSlides };
