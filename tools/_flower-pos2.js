...
flowerIds.forEach(id => {
  ...
  const bb = bboxEl(el);
  const cx = ((bb.minX + bb.maxX) / 2 / vbW * 100).toFixed(0);
  console.log(id, 'x~' + cx + '%', 'y', bb.minY.toFixed(0), '-', bb.maxY.toFixed(0));
});

console.log('=== key groups ===');
['g11102', 'g10734', 'g11470', 'path10138'].forEach(id => {
  const el = doc.getElementById(id);
  if (!el) return;
  const bb = bboxEl(el);
  console.log(id, 'x~' + ((bb.minX + bb.maxX) / 2 / vbW * 100).toFixed(0) + '%');
});

// top-level markup children
const root = doc.querySelector('g[transform]');
[...root.children].forEach(ch => {
  const bb = bboxEl(ch);
  console.log('top', ch.id || ch.tagName, 'x~' + ((bb.minX + bb.maxX) / 2 / vbW * 100).toFixed(0) + '%', 'paths', ch.querySelectorAll?.('path')?.length || 1);
});
