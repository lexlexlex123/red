...
function _expWave(pathStr,style,sw){try{var tmp=document.createElementNS('http://www.w3.org/2000/svg','svg');tmp.style.cssText='position:absolute;visibility:hidden;pointer-events:none;width:1px;height:1px;';document.body.appendChild(tmp);var p=document.createElementNS('http://www.w3.org/2000/svg','path');p.setAttribute('d',pathStr);tmp.appendChild(p);var tl=p.getTotalLength();document.body.removeChild(tmp);if(tl<1)return null;var hs=style==='wave'?sw*3.5:sw*2.5,amp=sw*0.85,nh=Math.max(4,Math.round(tl/hs));if(nh%2!==0)nh++;var ah=tl/nh,pts=[],mids=[];for(var i=0;i<=nh;i++){var pt=p.getPointAtLength(Math.min(tl,ah*i));pts.push({x:pt.x,y:pt.y});}for(var i=0;i<nh;i++){var pt=p.getPointAtLength(Math.min(tl,ah*i+ah*0.5));mids.push({x:pt.x,y:pt.y});}var d='M '+pts[0].x.toFixed(2)+' '+pts[0].y.toFixed(2)+' ';for(var i=0;i<nh;i++){var a=pts[i],b=pts[i+1],md=mids[i],dx=b.x-a.x,dy=b.y-a.y,len=Math.hypot(dx,dy),nx=len>0?-dy/len:0,ny=len>0?dx/len:1,side=(i%2===0)?1:-1,cpx=(md.x+nx*amp*side).toFixed(2),cpy=(md.y+ny*amp*side).toFixed(2);if(style==='wave')d+='Q '+cpx+' '+cpy+' '+b.x.toFixed(2)+' '+b.y.toFixed(2)+' ';else d+='L '+cpx+' '+cpy+' L '+b.x.toFixed(2)+' '+b.y.toFixed(2)+' ';}return d+'Z';}catch(e){return null;}}
function buildShapeSVG(d,w,h){var SHAPES=SHAPES_DATA;var sh=SHAPES.find(function(s){return s.id===d.shape;})||SHAPES[0];var op=d.fillOp===undefined?1:+d.fillOp;var fill=d.fill&&d.fill!=='none'?d.fill:(d.fill==='none'?'none':'#3b82f6');var hasFill=fill!=='none';var sw=d.sw===undefined?2:+d.sw;var sc=d.stroke||'#1d4ed8';var ss=d.strokeStyle||'solid';var isC=ss==='wave'||ss==='zigzag';var isDbl=ss==='double';var margin=(!isC&&sw>0)?sw/2:0;var shadow=d.shadow?'filter="url(#sh_'+d.id+')"':'';var fa=hasFill?'fill="'+fill+'"':'fill="none"';function shEl(fA,sA,m,ex){ex=ex||'';var ew=Math.max(1,w-m*2),eh=Math.max(1,h-m*2);if(sh.special==='rect')return '<rect x="'+m+'" y="'+m+'" width="'+ew+'" height="'+eh+'" rx="'+(d.rx||0)+'" '+fA+' '+sA+' '+ex+' '+shadow+'/>';if(sh.special==='ellipse')return '<ellipse cx="'+(w/2)+'" cy="'+(h/2)+'" rx="'+(ew/2)+'" ry="'+(eh/2)+'" '+fA+' '+sA+' '+ex+' '+shadow+'/>';if(sh.special==='callout')return null;var cnt=0,sx=ew/90,sy=eh/90,rawP=sh.path.split(' ').map(function(tok){if(tok==='M'||tok==='L'||tok==='Z')return tok;var v=parseFloat(tok);if(isNaN(v))return tok;var i=cnt++;return String(i%2===0?Math.round((v-5)*sx+m):Math.round((v-5)*sy+m));}).join(' ');var rx=d.rx||0,sp=rawP;if(rx>0){var pp=_expExtPts(rawP);if(pp.length>=3)sp=_expRndPath(pp,rx);}return '<path d="'+sp+'" '+fA+' '+sA+' '+ex+' '+shadow+'/>';}function shPS(m){var ew=Math.max(1,w-m*2),eh=Math.max(1,h-m*2);if(sh.special==='rect'){var rxr=d.rx||0;if(rxr>0)return 'M '+(m+rxr)+' '+m+' H '+(m+ew-rxr)+' Q '+(m+ew)+' '+m+' '+(m+ew)+' '+(m+rxr)+' V '+(m+eh-rxr)+' Q '+(m+ew)+' '+(m+eh)+' '+(m+ew-rxr)+' '+(m+eh)+' H '+(m+rxr)+' Q '+m+' '+(m+eh)+' '+m+' '+(m+eh-rxr)+' V '+(m+rxr)+' Q '+m+' '+m+' '+(m+rxr)+' '+m+' Z';return 'M '+m+' '+m+' H '+(m+ew)+' V '+(m+eh)+' H '+m+' Z';}if(sh.special==='ellipse'){var cx=w/2,cy=h/2,erx=ew/2,ery=eh/2;return 'M '+(cx-erx)+' '+cy+' A '+erx+' '+ery+' 0 1 1 '+(cx+erx)+' '+cy+' A '+erx+' '+ery+' 0 1 1 '+(cx-erx)+' '+cy+' Z';}if(sh.special==='callout')return null;var cnt2=0,sx=ew/90,sy=eh/90;return sh.path.split(' ').map(function(tok){if(tok==='M'||tok==='L'||tok==='Z')return tok;var v=parseFloat(tok);if(isNaN(v))return tok;var i=cnt2++;return String(i%2===0?Math.round((v-5)*sx+m):Math.round((v-5)*sy+m));}).join(' ');}var sd='',defB='';if(sh.special==='callout'){var sA2=sw>0?'stroke="'+sc+'" stroke-width="'+sw+'"':'stroke="none"';sd=_buildCalloutSVGPath(d,w,h,sh,fa,sA2,shadow,margin)||'';}else if(isC&&sw>0){var fm=sw/2,clipId='scp_'+d.id;if(hasFill){var cEl=shEl('fill="white"','stroke="none"',fm);defB+='<clipPath id="'+clipId+'">'+cEl+'</clipPath>';sd+=shEl(fa,'stroke="none"',fm,'clip-path="url(#'+clipId+')"')||'';}var ps=shPS(0),wp=ps?_expWave(ps,ss,sw):null;sd+=wp?'<path d="'+wp+'" fill="none" stroke="'+sc+'" stroke-width="'+sw+'" stroke-linecap="round" stroke-linejoin="round"/>':(shEl('fill="none"','stroke="'+sc+'" stroke-width="'+sw+'"',0)||'');}else if(isDbl&&sw>0){sd=(shEl(fa,'stroke="'+sc+'" stroke-width="'+(sw*3)+'"',sw*0.5)||'')+(shEl('fill="none"','stroke="'+fill+'" stroke-width="'+(sw*1.4)+'"',sw*0.5)||'');}else{var pm=_expPerim(sh,w,h,margin);var evD=(ss==='dotted'||ss==='dashed')?_expEvenDash(ss,sw,pm):'';var sA3=sw>0?'stroke="'+sc+'" stroke-width="'+sw+'" '+(evD||((ss==='dashed')?'stroke-dasharray="'+(sw*4)+' '+(sw*3)+'"':(ss==='dotted')?'stroke-dasharray="'+sw+' '+(sw*3)+'" stroke-linecap="round"':'')):'stroke="none"';sd=shEl(fa,sA3,margin)||'';}var fd='';if(d.shadow){var scol=d.shadowColor||'#000000',sb=d.shadowBlur||8,pad=Math.max(30,Math.ceil(sb*3));fd='<filter id="sh_'+d.id+'" x="-'+pad+'%" y="-'+pad+'%" width="'+(100+pad*2)+'%" height="'+(100+pad*2)+'%"><feDropShadow dx="3" dy="3" stdDeviation="'+sb+'" flood-color="'+scol+'" flood-opacity="0.6"/></filter>';}var defs=(fd||defB)?'<defs>'+(fd||'')+(defB||'')+'</defs>':'';return '<svg xmlns="http://www.w3.org/2000/svg" width="'+w+'" height="'+h+'" viewBox="0 0 '+w+' '+h+'" style="overflow:visible" opacity="'+op+'">'+defs+sd+'</svg>';}
