...
function openShapeModal(){
  ...
  var fill='#3b82f6',stroke='#1d4ed8';
  if(appliedThemeIdx>=0&&appliedThemeIdx<THEMES.length){
    var t=THEMES[appliedThemeIdx];
    fill=t.shapeFill||t.ac2||'#3b82f6';
    stroke=t.shapeStroke||t.ac1||'#1d4ed8';
  }else if(typeof getThemeAccents==='function'){
    var acc=getThemeAccents();fill=acc.ac2||fill;stroke=acc.ac1||stroke;
  }
  if(fillEl)fillEl.value=fill;
  if(strokeEl)strokeEl.value=stroke;
  var m=document.getElementById('shape-modal');if(m)m.classList.add('open');
}
function openShapeModal(){
  ...
  const m=document.getElementById('shape-modal');if(m)m.classList.add('open');
}
