...
function openShapeModal(){
  // Defaults: fill "15", stroke "14", thickness 0 (palette of the active theme).
  ...
  const fillColor=fillDef.color;
  const strokeColor=strokeDef.color;
  window._smFillScheme=fillDef.schemeRef||null;
  window._smStrokeScheme=strokeDef.schemeRef||null;
  const fillEl=document.getElementById('sm-fill');
  const strokeEl=document.getElementById('sm-stroke');
  ...
  const swEl=document.getElementById('sm-sw');
  if(swEl && !window._shapeReplaceMode) swEl.value=typeof DEFAULT_SHAPE_STROKE_W==='number'?DEFAULT_SHAPE_STROKE_W:0;
  buildShapeGallery();
...
function insertShapeSelected(){
  ...
  if(window._shapeReplaceMode&&sel&&sel.dataset.type==='shape'){
    ...
    if(d){
      ...
      if(_replIsCallout&&d.tailX===undefined){d.tailX=0;d.tailY=(d.h||200)/2+30;d.rx=d.rx||35;if(d.sw==null||d.sw===2)d.sw=14;}
      if(_replIsCallout&&!d.shapeTextCss) d.shapeTextCss='font-size:24px;font-weight:400;color:#ffffff;text-align:center;font-family:Boingster;';
      if(_replIsCallout&&d.shapeTextCss&&!/font-family/i.test(d.shapeTextCss))
        d.shapeTextCss+='font-family:Boingster;';
      if(_replIsCallout&&d.tailRoundX===undefined){
        const dr=typeof _calloutDefaultRoundRel==='function'?_calloutDefaultRoundRel(d.tailX||0,d.tailY||(d.h||200)/2+30):{tailRoundX:0,tailRoundY:90};
        d.tailRoundX=dr.tailRoundX;d.tailRoundY=dr.tailRoundY;
      }
      if(_replIsCallout&&d.tailWFrac===undefined) d.tailWFrac=0.2;
      if(_replIsCallout&&!d.calloutForm) d.calloutForm='round';
  ...
  const _insertSw = _isCloud ? 0 : (sh.noFill && !(sw>0) ? 2 : sw);
  ...
  const d={id:'e'+(++ec),type:'shape',x:snapV((canvasW-_insW)/2),y:snapV((canvasH-_insH)/2),w:snapV(_insW),h:snapV(_insH),
    shape:sh.id,fill:_insertFill,stroke:_isCloud?'#1d4ed8':stroke,sw:_insertSw,rx:_isCallout?35:0,fillOp:1,shadow:false,shadowBlur:4,shadowSize:3,shadowColor:'#000000',
    ...
    tailRoundX:_isCallout?0:undefined,tailRoundY:_isCallout?94:undefined,tailWFrac:_isCallout?0.2:undefined,
    calloutForm:_isCallout?'round':undefined,
    rot:0,anims:[],
...
window._buildCalloutSVGPath = _buildCalloutSVGPath;
window._calloutDefaultRoundRel = _calloutDefaultRoundRel;
...
/** SVG marker defs for line shape — center/tip anchored at path endpoint. */
function _lineMarkerDefHtml(id, type, color, atStart) {
  if (!type || type === 'none') return '';
  const col = color || '#1d4ed8';
  let inner = '';
  let refX = '1.5', refY = '1.5', orient = 'auto', mw = '3.0', mh = '3.0';
  if (type === 'arrow') {
    mw = '3.1'; mh = '3.5';
    ...
    orient = '0';
    inner = `<path d="M0.3,0.3 L2.7,2.7" stroke="${col}" stroke-width="1" stroke-linecap="round" fill="none"/>`
...
window._lineMarkerDefHtml = _lineMarkerDefHtml;

const _LINE_MARKER_TYPES = ['none', 'arrow', 'square', 'circle', 'bar', 'cross'];
const _LINE_MARKER_SVGS = {
  none: '<svg viewBox="0 0 28 28" fill="none"><line x1="4" y1="14" x2="24" y2="14" stroke="currentColor" stroke-width="2" stroke-linecap="round"/></svg>',
  arrow: '<svg viewBox="0 0 28 28" fill="none"><line x1="4" y1="14" x2="17" y2="14" stroke="currentColor" stroke-width="2" stroke-linecap="round"/><path d="M16.5,12.1 Q20,14 16.5,15.9 L13.2,17.8 Q10,19.5 10,17.2 L10,10.8 Q10,8.5 13.2,10.2 Z" fill="currentColor"/></svg>',
  square: '<svg viewBox="0 0 28 28" fill="none"><line x1="4" y1="14" x2="18" y2="14" stroke="currentColor" stroke-width="2" stroke-linecap="round"/><rect x="18" y="9" width="10" height="10" rx="1" stroke="currentColor" stroke-width="2" fill="currentColor" opacity="0.25"/></svg>',
  circle: '<svg viewBox="0 0 28 28" fill="none"><line x1="4" y1="14" x2="17" y2="14" stroke="currentColor" stroke-width="2" stroke-linecap="round"/><circle cx="22" cy="14" r="5" stroke="currentColor" stroke-width="2" fill="currentColor" opacity="0.25"/></svg>',
  bar: '<svg viewBox="0 0 28 28" fill="none"><line x1="4" y1="14" x2="22" y2="14" stroke="currentColor" stroke-width="2" stroke-linecap="round"/><line x1="22" y1="7" x2="22" y2="21" stroke="currentColor" stroke-width="2" stroke-linecap="round"/></svg>',
  cross: '<svg viewBox="0 0 28 28" fill="none"><line x1="4" y1="14" x2="17" y2="14" stroke="currentColor" stroke-width="2" stroke-linecap="round"/><line x1="19" y1="8" x2="25" y2="20" stroke="currentColor" stroke-width="2" stroke-linecap="round"/><line x1="25" y1="8" x2="19" y2="20" stroke="currentColor" stroke-width="2" stroke-linecap="round"/></svg>'
};

function _updateLineMarkerButtons(from, to) {
  ['from', 'to'].forEach(side => {
    const group = document.getElementById('sh-marker-' + side);
    if (!group) return;
    const val = side === 'from' ? (from || 'none') : (to || 'none');
...
function _initLineMarkerButtons() {
  if (document.getElementById('sh-line-markers-inited')) return;
  ...
  ['from', 'to'].forEach(side => {
    const host = document.getElementById('sh-marker-' + side);
    if (!host) return;
    host.innerHTML = '';
    host.style.cssText = 'display:flex;gap:3px;flex-wrap:wrap;';
    _LINE_MARKER_TYPES.forEach(mk => {
      const btn = document.createElement('button');
      btn.type = 'button';
      btn.className = 'mk-btn';
      btn.dataset.mk = mk;
      btn.title = mk;
      btn.style.cssText = 'width:34px;height:34px;border-radius:6px;border:1.5px solid transparent;background:transparent;cursor:pointer;display:flex;align-items:center;justify-content:center;padding:3px;flex-shrink:0;color:var(--text,#1e293b);';
      btn.innerHTML = _LINE_MARKER_SVGS[mk] || '';
      btn.addEventListener('click', () => setLineMarker(side, mk));
      host.appendChild(btn);
    });
  });
  const flag = document.createElement('span');
  flag.id = 'sh-line-markers-inited';
  flag.hidden = true;
  document.body.appendChild(flag);
}
window._initLineMarkerButtons = _initLineMarkerButtons;
window._updateLineMarkerButtons = _updateLineMarkerButtons;

function setLineMarker(side, mk) {
  if (!sel || sel.dataset.type !== 'shape' || sel.dataset.shape !== 'line') return;
  const prop = side === 'from' ? 'lineFromMarker' : 'lineToMarker';
  updateShapeStyle(prop, mk === 'none' ? 'none' : mk);
  const d = slides[cur] && slides[cur].els.find(e => e.id === sel.dataset.id);
  _updateLineMarkerButtons(d && d.lineFromMarker, d && d.lineToMarker);
}
window.setLineMarker = setLineMarker;
...
function buildShapeSVG(d, w, h) {
  ...
  if (sh.special === 'callout') {
    const calloutM = sw > 0 ? Math.max(margin, sw * 0.9) : margin;
    const sAttr = sw > 0
      ? `stroke="${strokeColor}" stroke-width="${sw}" ${_getStrokeDasharray(strokeStyle, sw)}`
      : 'stroke="none"';
    shapeDef = _buildCalloutSVGPath(d, w, h, sh, fillAttr, sAttr, shadow, calloutM);
  ...
  // Line endpoint markers — defs must be ready before <defs> is assembled
  let lineMkDefs = '';
  if (sh.id === 'line') {
    const fromMk = d.lineFromMarker || 'none';
    const toMk = d.lineToMarker || 'none';
    const canMk = !isComplex && !isDouble && (fromMk !== 'none' || toMk !== 'none');
    if (canMk) {
      const mkP = 'lm_' + d.id;
      if (fromMk !== 'none') lineMkDefs += _lineMarkerDefHtml(mkP + '_f', fromMk, strokeColor, true);
      if (toMk !== 'none') lineMkDefs += _lineMarkerDefHtml(mkP + '_t', toMk, strokeColor, false);
      const y = h / 2;
      const perim = Math.max(1, w);
      const evenD = (strokeStyle === 'dotted' || strokeStyle === 'dashed')
        ? _evenDash(strokeStyle, sw, perim) : null;
      let sAttr = evenD
        ? `stroke="${strokeColor}" stroke-width="${sw}" stroke-dasharray="${evenD.dasharray}" ${evenD.extraAttrs}`
        : `stroke="${strokeColor}" stroke-width="${sw}" ${_getStrokeDasharray(strokeStyle, sw)}`;
      sAttr += ' stroke-linecap="butt"';
      let mkAttr = '';
      if (fromMk !== 'none') mkAttr += ` marker-start="url(#${mkP}_f)"`;
      if (toMk !== 'none') mkAttr += ` marker-end="url(#${mkP}_t)"`;
      shapeDef = `<path d="M 0 ${y} L ${w} ${y}" fill="none" ${sAttr}${mkAttr} ${shadow}/>`;
    }
  }
  ...
  if (lineMkDefs) defBlock = lineMkDefs + defBlock;
...
function renderShapeEl(el,d,opts){
  ...
  // Текст: пересчитать padding/центр при ресайзе
  const _txtLay=el.querySelector('.shape-text');
  if(_txtLay&&typeof _layoutShapeText==='function') _layoutShapeText(_txtLay,d,el);
...
function updateShapeStyle(prop,val){
  ...
  else if(prop==='lineFromMarker'){
    const v=val||'none';
    if(v==='none'){ delete d.lineFromMarker; delete sel.dataset.lineFromMarker; }
    else { d.lineFromMarker=v; sel.dataset.lineFromMarker=v; }
  }
  else if(prop==='lineToMarker'){
    const v=val||'none';
    if(v==='none'){ delete d.lineToMarker; delete sel.dataset.lineToMarker; }
    else { d.lineToMarker=v; sel.dataset.lineToMarker=v; }
  }
  renderShapeEl(sel,d);save();drawThumbs();saveState();
...
/**
 * Вертикальное центрирование текста фигуры.
 * Область текста всегда по центру элемента — не зависит от хвоста выноски.
 */
function _layoutShapeText(txt, d, el) {
  if (!txt || !d) return;
  const h = Math.max(1, +(d.h || (el && parseFloat(el.style.height)) || 100));
  const w = Math.max(1, +(d.w || (el && parseFloat(el.style.width)) || 100));
  const cs = (d.shapeTextCss || txt.getAttribute('style') || '');
  const fs = parseFloat((cs.match(/font-size:\s*([\d.]+)/) || [])[1] || 24);
  const sh = typeof SHAPES !== 'undefined' && SHAPES.find(s => s.id === d.shape);
  const isCallout = sh && sh.special === 'callout';
  const sw = Math.max(0, +(d.sw != null ? d.sw : 2));
  ...
  txt.style.padding = padY + 'px ' + padX + 'px';
  txt.style.overflow = 'hidden';
  // Равномерный inset (для выноски — от обводки); без сдвига от tip
  const edge = isCallout ? Math.max(2, Math.round(sw * 0.45)) : 0;
  txt.style.inset = edge + 'px';
  txt.style.top = txt.style.right = txt.style.bottom = txt.style.left = '';

  const inner = txt.querySelector(':scope > div') || txt.firstElementChild;
  if (inner && inner.style) {
    inner.style.transform = 'translateY(-0.07em)';
    inner.style.lineHeight = '1.15';
    inner.style.margin = '0';
    inner.style.padding = '0';
  }
}
window._layoutShapeText = _layoutShapeText;
...
/** Оставить в CSS только текстовые свойства (без layout от _layoutShapeText). */
function _shapeTextCssPick(fromStyle, prev) {
  ...
  return parts.length ? parts.join(';') + ';' : (prev || 'font-size:24px;font-weight:700;color:#ffffff;text-align:center;');
}
window._shapeTextCssPick = _shapeTextCssPick;
function updateShapeTextStyle(prop,val){
  ...
  // Типографика через CSSOM — не затирает layout от _layoutShapeText
